...
class FeishuNotifier:
    def __init__(self, webhook: Optional[str] = None, app_name: str = "feishu-bitable"):
    ...
    def interactive(self, title: str, lines: List[str], color: str = "blue") -> bool:
        ...
        elements.append(
            {
                "tag": "note",
                "elements": [{"tag": "plain_text", "content": f"{self.app_name} · 自动告警"}],
            }
        )
        payload = {
    ...
    def run_summary(
        ...
        title = f"{'✅' if ok else '⚠️'} {self.app_name} 跑完"
        lines = [
...
def get_notifier(app_name: str = "feishu-bitable") -> FeishuNotifier:
    global _notifier
    if _notifier is None or _notifier.app_name != app_name:
        _notifier = FeishuNotifier(app_name=app_name)
    return _notifier