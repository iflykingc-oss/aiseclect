"""
LLM 客户端（OpenAI 兼容协议，可接豆包 / DeepSeek / OpenAI 等）
- 配置来源（按优先级）：
  1. 入参 explicit
  2. 环境变量 OPENAI_API_KEY / OPENAI_BASE_URL / OPENAI_MODEL
  3. heat_scorer_llm_cfg.json / tweet_generator_llm_cfg.json 中的 config 段
"""
...
def invoke_with_retry(model, messages, max_attempts: int = 3, base_delay: float = 1.0):
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            logger.warning(f"LLM invoke 失败（第 {attempt}/{max_attempts} 次），{delay:.1f}s 后重试: {type(e).__name__}: {str(e)[:200]}")
            time.sleep(delay)
