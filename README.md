...
AI 资讯采集 → 双平台推文生成（X + 小红书） → 飞书 Bitable 写入（人工审核）。
...
| 热度评分 | 数字 | 自动 | 0-100 |
| 推文内容 | 文本 | 自动 | X 平台 ≤280 字符 |
| 独立观点 | 文本 | 自动 | LLM 观点 |
| 小红书标题 | 文本 | 自动 | 带 emoji |
| 小红书内容 | 文本 | 自动 | 200-300 字（仅X 时留空） |
| 小红书标签 | 文本 | 自动 | 逗号分隔（仅X 时留空） |
| **发布平台** | 单选 | 自动 | **X+小红书 / 仅X**（LLM 决策） |
| 处理状态 | 单选 | 自动「待审核」 | 待审核 / 已通过 / 已驳回 / 已发布 |
| **人工审核结果** | 单选 | **手填** | 通过 / 需修改 / 驳回 |
| **审核备注** | 文本 | **手填** | 批注 |
| 创建时间 | 日期 | 自动 | 写入时间 |
...
2. 你打开飞书表格看推文草稿
3. 把「人工审核结果」改成「通过 / 需修改 / 驳回」
4. 「审核备注」写修改原因（可选）
5. 审核通过的，按「发布平台」字段决定复制到哪些平台（仅X 时不用管小红书那三列）
6. 发布后把「处理状态」改成「已发布」
